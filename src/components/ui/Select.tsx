...
import { MouseEvent, ReactNode, createContext, useContext, useEffect, useId, useRef, useState } from "react";
...
function Select(props: SelectProps) {
    ...
    const onUpdate = (option: SelectOption) => {
        setOptions((options)=>options.filter((elem)=>elem.id!==option.id).concat([option]))
    }
    ...
    return (
        <SelectContext.Provider value={{options, value: props.value, onChange: props.onChange, onUpdate}}>
            <motion.div ref={rootRef} onClick={onParentClick} className={cursor}>
                <TextInput
                ...
                <motion.div
                    ...
                    transition={{opacity: {delay: isOpened?0.20:0}, scale:{type:"tween", delay: isOpened?0:0.20}}}
...
Select.Option = (props: SelectOptionProps) => {
    ...
    return (
        <p onClick={()=>context.onChange(props.value)} className="p-2 bg-white cursor-pointer hover:bg-gray-200">
            {props.label}
        </p>
    )
}
